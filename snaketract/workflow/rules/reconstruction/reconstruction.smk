...
def _select_fod_algorithm(wcards):
    if (
        config.get('single_shell_algorithm', 'ss3t') == "csd"
...
rule run_act:
    ...
    params:
        maxlength=config['tractography_maxlength'],
        cutoff=config['tractography_cutoff'],
        num_tracts=int(config['tractography_numtracts']),
        rec=lambda wcards: {"iFOD2": "iFOD2"}[wcards['rec']]
    threads: 32
    resources:
        runtime=int(max(int(config['tractography_numtracts']) * 90 / 10_000_000, 10)),
        mem_mb=4000,
    log: log("run_act", inputs['preproc_dwi'], "rec")
    benchmark: benchmark("run_act", inputs['preproc_dwi'], "rec")
    envmodules:
        "mrtrix/3.0.1",
...
rule run_sift2:
    ...
    envmodules:
        "mrtrix/3.0.1",
        "git-annex/8.20200810"
    log: log("run_sift2", inputs['preproc_dwi'], "rec")
    benchmark: benchmark("run_sift2", inputs['preproc_dwi'], "rec")
    group: "sift"
    shell:
